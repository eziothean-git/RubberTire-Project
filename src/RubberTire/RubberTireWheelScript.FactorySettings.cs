...
public partial class RubberTireWheelScript
{
    ...
    private List<RubberTireFactorySetting> BuildFactorySettings()
    {
        ...
        s.Add(FactoryBool("Tire", "mlsAD", "Low-speed axle damping", delegate { return enableLowSpeedAxleDamping; }, delegate(bool v) { enableLowSpeedAxleDamping = v; }).In("Low speed").When(whenModernLowSpeed).Tip("Passive finite-strength replacement for the stock wheel motor's zero-speed hold"));
        s.Add(FactoryFloat("Tire", "mlsADS", "Axle damping fade speed (m/s)", 0.1f, 10f, delegate { return lowSpeedAxleDampingMaxSpeed; }, delegate(float v) { lowSpeedAxleDampingMaxSpeed = v; }).In("Low speed").Adv().When(delegate { return enableModernLowSpeedTire && enableLowSpeedAxleDamping; }));
        s.Add(FactoryFloat("Tire", "mlsADT", "Axle damping time (s)", 0.01f, 1f, delegate { return lowSpeedAxleDampingTime; }, delegate(float v) { lowSpeedAxleDampingTime = v; }).In("Low speed").Adv().When(delegate { return enableModernLowSpeedTire && enableLowSpeedAxleDamping; }));
        s.Add(FactoryFloat("Tire", "mlsADF", "Axle damping grip fraction", 0f, 0.5f, delegate { return lowSpeedAxleDampingGripFraction; }, delegate(float v) { lowSpeedAxleDampingGripFraction = v; }).In("Low speed").Adv().When(delegate { return enableModernLowSpeedTire && enableLowSpeedAxleDamping; }).Tip("Maximum damping torque as a fraction of current static tyre capacity"));
